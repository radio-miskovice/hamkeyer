# hamkeyer — serial keyer interface for browser and NodeJS

Primary purpose of this package is to provide library for 
interaction with Winkeyer keyers (K1EL, K3NG and maybe some more). 
The main reason is that Winkeyer protocol is rather complicated 
and every developer of any program that needs to communicate
with "a" keyer has to implement at least the most crucial parts 
of it to make their program useful for CW operator.

Second reason is to encapsulate the weirdness of Winkeyer protocol
into a more-less standard API pattern.

Third reason is that there are, although rare, keyers using another protocol (e.g. [SpiderKeyer](https://ok1fig.nagano.cz/SpiderKeyer/SpiderKeyer.htm)) not supported by mainstream logger 
developers. The abstraction layer in `cwkeyer` API makes it possible
to add Spider Keyer protocol adapter to this package and thus offer a more universal tool to other developers.

This package provides reduced API to cover only the most basic functions of Winkeyer - send text, 
manage keying speed and a few more features. The main focus is on
computer aided operation, so setup of manual keying functions is 
limited, as well as PTT management. This is because the author believes 
that transceiver control is mostly managed by CAT and hence need
not be done by the keyer itself.

In the future some hardware-related features will be added that are
common in different keyer protocols and must be addressed to
overcome timing aspects of the keyed radio equipment.
